...
| **다국어 지원** | 한국어 및 영어 지원 (설정 UI 및 트레이 메뉴) |
| **다크 모드** | 시스템 설정 및 수동 토글을 통한 다크/라이트 테마 지원 |
| **설정 UI** | 트레이 메뉴 → `localhost` 브라우저 UI에서 저장 경로, 단축키, 언어 등 설정 |
...
| 테마 및 설정 | 서버사이드 JSON 설정 관리 (localStorage 미사용) |
...
│   ├── config/               # 설정 파일 및 관리자
│   └── utils/                # 로거 등 공통 유틸리티
├── build-work/               # 빌드 및 패키징 스크립트
├── doc/                      # 작업 기록 및 설계 문서 (done 폴더 참조)
└── ref/                      # 참고 자료
```
...
| **노캐시 정책** | 설정 UI 변경 사항이 즉각 반영되도록 모든 API/정적 파일에 노캐시 헤더 적용 |
...
| WebServer | Gin 기반 로컬 HTTP 서버 (Port: 18420), 설정 UI 및 API 제공 |
...
| 경로 | 메서드 | 설명 |
|------|--------|------|
| `/api/config` | GET | 현재 설정값(경로, 단축키, 언어, 테마 등) 반환 |
| `/api/config` | POST | 설정값 갱신 |
| `/api/config/reset` | POST | 설정을 기본값(`default-config.json`)으로 초기화 |
| `/api/darkmode` | POST | 다크모드 상태 별도 저장 |
| `/api/permissions`| GET | macOS 화면 기록 및 손쉬운 사용 권한 상태 확인 |
| `/api/buildtime` | GET | 빌드 시점 정보 반환 |
